...
const Bill = () => {
  ...
  const handleOpenTable = () => {
    ...
    const foundTable = tables.find((table) => table.tableName === inputTableNumber);
    ...
    if (foundTable) {
      ...
      console.log(`Table with name ${inputTableNumber} not found.`);
    }
  };
    ...
    const fetchData = async () => {
      try {
        const sectionsResponse = await axios.get(
          "https://backdeploy.vercel.app/api/section"
        );
        setSections(sectionsResponse.data);

        const tablesResponse = await axios.get(
          "https://backdeploy.vercel.app/api/table/tables"
        );
        ...
        const billsData = await Promise.all(
          tablesResponse.data.map(async (table) => {
            const billsResponse = await axios.get(
              `https://backdeploy.vercel.app/api/order/order/${table._id}`
            );
